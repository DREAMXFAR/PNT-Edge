...
def convert_mat_to_numpy(label_path, h, w, cls_num=20):
...
class SBDTrainData(data.Dataset):
    def __init__(self, img_folder, label_folder, anno_txt, input_size, cls_num, img_transform, label_transform, thinpb=False):
        ...
        for line in lines:
            row_data = line.split()
            img_name = row_data[0][1:]
            label_name = row_data[1][1:]
    ...
    def __getitem__(self, index):
        ...
        same_seeds(seed)  # to keep same transformation for image and labels
        processed_img = self.img_transform(img)  # 3 X H X W

        ### from mat to numpy
        np_data = convert_mat_to_numpy(label_path, h, w, cls_num=20)

        label_data = []
        num_cls = np_data.shape[2]
        for k in range(num_cls):
            same_seeds(seed)  # Before transform, set random seed same as img transform, to keep consistent!
...
class SBDValData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        same_seeds(seed)  # set same seed for transformation of label and image
        processed_img = self.img_transform(img)  # 3 X H X W

        img_info = {"img_name": img_name, "orig_size": (w, h)}

        return processed_img, img_info

    def __len__(self):
        return len(self.ids)


class SBDTrainDataWithCanny(data.Dataset):
    def __init__(self, img_folder, label_folder, anno_txt, input_size, cls_num, img_transform, label_transform, thinpb=False):
        self.img_folder = img_folder
        self.label_folder = label_folder
        self.canny_folder = r'./example/my_labels/canny/'
        self.input_size = input_size
        self.cls_num = cls_num
        self.img_transform = img_transform
        self.label_transform = label_transform
        ...
        for line in lines:
            ...
            self.idx2name_dict[cnt]['img'] = img_name
            self.idx2name_dict[cnt]['label'] = label_name
            self.idx2name_dict[cnt]['canny'] = label_name.replace('label_inst_orig', self.canny_folder).replace('mat', 'png')
            self.ids.append(cnt)

            cnt += 1

    def get_canny(self, img, radius=5, sigma=1.5, low_thr=25, high_thr=150):
        img = img.convert('L')
        img = np.array(img)
        img = cv2.GaussianBlur(img, (radius, radius), sigma)  # sigmaX=2, sigmaY=2
        edges = cv2.Canny(img, low_thr, high_thr, apertureSize=3, L2gradient=True)  # Canny, apertureSize=3(default)
        return edges
    ...
    def __getitem__(self, index):
        ...
        # canny_gt = self.get_canny(img)  # canny online
