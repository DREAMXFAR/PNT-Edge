...
def load_npy_to_layer(model, layer_to_name_dict, npy_folder, loaded_model_path, convert_on=True):
    ...
    for (m_name, m_data) in model.state_dict().items():
        if m_name in layer_to_name_dict:
            if not ("running_mean" in m_name or "running_var" in m_name):
                ...
                np_data = np.load(open(npy_path, 'rb'))  # the numpy is stored in binary file, by xwj 22-09-16
                ...
                up_data = np.load(open(up_path, 'rb'))
                down_path = os.path.join(npy_folder, layer_to_name_dict[m_name][1]+".npy")
                down_data = np.load(open(down_path, 'rb'))
                pretrained_dict[m_name] = torch.from_numpy(up_data/down_data)
                print("{0} loaded successfully".format(m_name))
        else:
            if not "num_batches_tracked" in m_name:
                print('==========================> {} not loaded!'.format(m_name))

    # Store model into pth file.
    model.load_state_dict(pretrained_dict)
    if convert_on:  # added by xwj
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=19):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3,
                               bias=False)
        self.bn_conv1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        set_require_grad_to_false(self.bn_conv1)
   
        self.relu = nn.ReLU(inplace=True)
        # self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=0,)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=0, ceil_mode=False)  # checked by xwj, change back to false for 472. 474 not match concat_feats()
...
def CASENet_resnet101(pretrained=False, num_classes=19):
    ...
    if pretrained:
        utils.load_official_pretrained_model(model, "/path/to/model_casenet.pth.tar")
    return model


if __name__ == "__main__":
    model = CASENet_resnet101(pretrained=False, num_classes=20)  # 19 classes for Cityscapes
