...
def convert_num_to_bitfield(label_data, h, w, root_folder, label_png_name, cls_num=19):
    ...
    for cls_idx in range(cls_num):
        imsave(os.path.join(root_folder, label_png_dir, str(cls_idx)+'.png'), all_bit_tensor_list[:,:,cls_idx].numpy()*255)


if __name__ == "__main__":
    from tqdm import tqdm

    f = open("./CASENet-master/cityscapes-preprocess/data_proc/train.txt", 'r')
    lines = f.readlines()
    root_folder = "./CASENet-master/cityscapes-preprocess/data_proc"
    cnt = 0
